...
;      2017 July,  K. Cho : add Nowin keyword
;-
function alignoffset, image, reference, cor, nowin=nowin, cor1=cor1
                                   ;  Check compatibility

si = size(image) ; [n_elements(image[*,0]), n_elements(image[0,*])]
sr = size(reference) ; [n_elements(reference[*,0]), n_elements(reference[0,*])]
...
if keyword_set(nowin) then window=1.
;stop

cor1 = float(fft(fft(image1*window, 1)*fft(reference1*window, -1), -1))
...
cc = (shift(cor1, -x0+1, -y0+1))(0:2, 0:2)
