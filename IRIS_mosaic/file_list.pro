function file_list, dirs, wild_str, interactive=interactive, files=file, fdir=fdir, $
...
;	wild_str	- The wildcard partial filename to search for.  Be careful
...
if (keyword_set(interactive)) then begin
    wild_str = ''
    read, 'Enter file to search for - use wildcards (ie: sfr*) ', wild_str
end else begin
    if (n_elements(wild_str) eq 0) then wild_str = '*'
end
;
ff = ''
...
for i=0,n_elements(dirs)-1 do begin
    ...
    ;
    for j=0,n_elements(wild_str)-1 do begin
        dot = strpos (wild_str(j), '.')
        if ((dot eq -1) and (!version.os eq 'vms')) then wild_str(j) = wild_str(j) + '.*'
	if (not keyword_set(cd)) then begin
	    ff0 = findfile( concat_dir(dir, wild_str(j)) )	;original technique - works well if the directory might not exist
	end else begin
	    if (keyword_set(bydate)) then spawn, ['ls', '-tr', dir], all_files, /noshell	$
				else all_files = findfile(dir)			;get all files in that directory
	    ss = wc_where(all_files, wild_str(j))
	    if (ss(0) eq -1) then ff0 = '' else ff0 = concat_dir(dir, all_files(ss))
            ;;if (dirs(i) eq '') then cd, start_dir else cd, dirs(i)
            ;;ff0 = findfile(wild_str(j))                              ;do not include directory in the listing
...
if (n_elements(ff) eq 1) then begin
    ...
    ff = ff(1:*)	;wild_strip off first element
    ;
    n = n_elements(ff)
    file = strarr(n)
    fdir = strarr(n)
