...
;                Cho. K (SNU) 20 may 2013 
;                - change float to double (to use Julian Day)
;                Cho. K (SNU) 10 Jan 2014
;                - enable to noxtick = 0 and noytick = 0  
;                Cho. K (SNU) 16 May 2015
;                - cut pixels outside plot window range
;                Cho. K (SNU) 25 Sep 2015
;                - change the defalut value of truncate
; Contact     : dzarro@solar.stanford.edu
;-

pro plot_map_d,map,index,contour=cont,overlay=overlay,smooth_width=smooth_width,border=border,$
...
units=get_map_prop(map[index],/units,def='arcsec')
...
if ~keyword_set(truncate) then truncate=1
...
if exist(fov) then begin
 if valid_map(fov) then dfov=get_map_fov(fov,/edge) else begin
  nfov=n_elements(fov)
  dfov=60.*double([fov[0],fov[nfov-1]])
 endelse
 half_fov=dfov/2.
...
if exist(drange) then begin
 if valid_map(drange) then prange=get_map_drange(drange) else $
  if valid_range(drange) then prange=double(drange)
endif
...
if ~over then begin
 ...
 if square_scale and ~do_multi and ~have_tag(extra,'pos',/start) then begin
  dpos=get_aspect(xrange=dxrange,yrange=dyrange,_extra=extra)
  extra=rep_tag_value(extra,dpos,'pos')
 endif
 if square_scale and do_multi and ~have_tag(extra,'pos',/start) then begin  
...
; endif
 if have_tag(extra,'noxt') then begin
  if (extra.(where(strmatch(tag_names(extra), 'noxt*', /fold_case) ne 0))) then begin
  xticks=replicate(' ',n_elements(!x.tickname)-1)
  extra=rep_tag_value(extra,xticks,'xtickname')
  endif
 endif
 if have_tag(extra,'noyt') then begin
  if (extra.(where(strmatch(tag_names(extra), 'noyt*', /fold_case) ne 0))) then begin
  yticks=replicate(' ',n_elements(!y.tickname)-1)
  extra=rep_tag_value(extra,yticks,'ytickname')
  endif
 endif
...
if cont then begin
 ...
 if n_elements(levels) gt 0 then begin
  plevels=get_uniq(double(levels))
...
;-- dimensions of scaled image
 ...
 sx=(abs(max(xb_dev)-min(xb_dev))+1d) > 1d
 sy=(abs(max(yb_dev)-min(yb_dev))+1d) > 1d
...
;; added by K. Cho
lcount=floor(xr_dev[0])-floor(xb_dev[0])
rcount=floor(xb_dev[0])+floor(sx-1)-floor(xr_dev[1])
bcount=floor(yr_dev[0])-floor(yb_dev[0])
tcount=floor(yb_dev[0])+floor(sy-1)-floor(yr_dev[1])

;stop
if lcount gt 0 then pic=pic[lcount:*, *]
if rcount gt 0 then pic=pic[0:-rcount-1, *]
if bcount gt 0 then pic=pic[*, bcount:*]
if tcount gt 0 then pic=pic[*, 0:-tcount-1]

;stop
xb_dev[0]=xb_dev[0]+lcount
yb_dev[0]=yb_dev[0]+bcount
...
;-- if Postscript, we literally "white-out" pixels outside viewport
 ...
 if post then begin
  tv,pic,xb_dev[0]+1,yb_dev[0]+1,xsize=sx-1,ysize=sy-1,/device
...
if ~over then begin
 if square_scale and do_multi and ~have_tag(extra,'pos',/start) then setscale
endif 
