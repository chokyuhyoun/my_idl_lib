...
;               that specifies the absolute distance(s) between
;               indexed elements of X. It can be floats. 
;               If not, as a default, It would be [-(n-2), (n-2)] integer. 
;
;
...
;-
function C_Correlate_k, X, Y, Lag=Lag, Covariance = Covariance, Double = doubleIn
...
;    ON_ERROR, 2
    ...
    if n_elements(lag) eq 0 then lag = [-nx/2:nx/2:1] 
...
;            TOTAL(Yd[0:nX + Lag[k] - 1L] * Xd[-Lag[k]:*])
      locx = findgen(nx) - lag[k]
      locx = locx[where(locx ge 0 and locx le nx-1)]
      locy = findgen(nx) + lag[k]
      locy = locy[where(locy ge 0 and locy le nx-1)]
      cross[k] = total(interpol(xd, findgen(nx), locx) $
                      *interpol(yd, findgen(nx), locy)) 
    endfor
;stop
