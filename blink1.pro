...
;
	window, /free, retain=2, xsize = max(ncol), ysize=max(nrow), $
                   xpos=0, ypos=0, $ 
                   title="Blink window - Press q key to exit"
        whd = !d.window
	i = 0L
...
REPEAT BEGIN
   ...
   device, copy=[0, 0, ncol[i], nrow[i], 0, 0, wndw[i]]
   wset, whd
   xyouts, 0.1, 0.1, string(wndw[i], f='(i0)'), /nor, charsize=2, charthick=4, color=255
   xyouts, 0.1, 0.1, string(wndw[i], f='(i0)'), /nor, charsize=2, charthick=2, color=0
   wait, 0.01
ENDREP UNTIL A eq 'q'
