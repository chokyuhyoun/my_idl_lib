...
amp=par[1]
disp=1.; par[2]
f=convol(-amp*fiss_tell_lines_cor((wv-median(wv))*disp+median(wv)+dwv), [-1,1])
end

function fiss_tell_rm_1s, wv, sp, par, nofit=nofit

;+
;  Name: FISS_TELL_RM_1s
;  Purpose:
...
;      2013 K. Cho corrected (can be used only one profile)
...
res=curvefit(wv,  y,  fiss_tell_lines_cor(wv) ge 0.02, par,  /noderivative, funct='fiss_tell_model')
endif
model=sp*(exp(par[1]*fiss_tell_lines_cor((wv-median(wv))+median(wv)+par[0]))#replicate(1., n_elements(sp[0,*])))
;stop
return, model
end
